    ...
    function loop(ts) {
      ...
      g.seeds = g.seeds.filter(s => {
        if (d2(p, s) < 22) {
          ...
          g.truthIdx++
          const cx = Math.max(0.12, Math.min(0.85, p.x / g.W))
          const cy = Math.max(0.06, Math.min(0.78, p.y / g.H))
          setSeedBubble({ text: truth, cx, cy, id: g.score })
          return false
        }
        return true
      })
        ...
        <div className="vp-canvas-wrap" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
          ...
          {seedBubble && (
            <div
              ...
              key={seedBubble.id}
              style={{ left: `${seedBubble.cx * 100}%`, top: `${seedBubble.cy * 100}%` }}
