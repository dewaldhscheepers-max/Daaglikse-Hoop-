...
export default function DinkNuutLeefNuut({ onClose }) {
  const [view, setView]             = useState('list')
  const [currentDay, setCurrentDay] = useState(null)
  const [dae, setDae]               = useState(cachedDae || [])
  const [loading, setLoading]       = useState(!cachedDae)
  const [completed, setCompleted]   = useState(() => {
    try { return JSON.parse(localStorage.getItem('dnln_completed') || '[]') } catch { return [] }
  })
  const [lastDay, setLastDay]       = useState(() => {
    const v = parseInt(localStorage.getItem('dnln_lastDay') || '0')
    return v || null
  })
  ...
  useEffect(() => {
    if (cachedDae) return
    fetch('/dink-nuut-leef-nuut.json')
      .then(r => r.json())
  ...
  function markCompleted(dagNr) {
    ...
    setCompleted(next)
    localStorage.setItem('dnln_completed', JSON.stringify(next))
  }

  function handleShare() {
    sharePlan('Dink Nuut, Leef Nuut', `💡 Dink Nuut, Leef Nuut — 5 dae\n\nLees dit gratis op Daaglikse Hoop:`)
  }

  function openDay(dagNr) {
    const dag = dae.find(d => d.dag === dagNr)
    if (!dag) return
    if (!lastDay && !localStorage.getItem('rp_counted_dnln')) {
      localStorage.setItem('rp_counted_dnln', '1')
      fetch('/api/kinder-boek-read', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ bookId: 'dink-nuut-leef-nuut' }),
      }).catch(() => {})
    }
    setCurrentDay(dag)
    setLastDay(dagNr)
    localStorage.setItem('dnln_lastDay', String(dagNr))
    setView('day')
  }
      ...
      <div className="dvv-overlay">
        <div className="dvv-screen">
          <div className="dvv-header">
            ...
            <span className="dvv-header-title">Dink Nuut, Leef Nuut</span>
          ...
          <div className="dvv-list-body" ref={bodyRef}>
            <div className="bmg-intro">
              <div className="bmg-intro-sub">5 dae · Gedagte-hernuwing deur Dewald Scheepers</div>
      ...
      <div className="dvv-overlay">
        <div className="dvv-screen" ref={bodyRef}>
          ...
          <div className="dvv-closing">
            ...
            <h1 className="dvv-closing-title">Jy het jou denke begin vernuwe</h1>
            <p className="dvv-closing-body">
              Jou lewe verander nie oornag nie — dit verander gedagte vir gedagte. En jy het begin.
            </p>
            <p className="dvv-closing-body">
              Jy het geleer om te toets, te vang, te rig en in praktyk te sit. Die pad vorentoe is dieselfde: elke keer as 'n leuen kom, antwoord eenvoudig — "Daar staan geskrywe."
            </p>
            <p className="dvv-closing-body">
              "Word verander deur die vernuwing van julle gemoed." — Romeine 12:2
            </p>
