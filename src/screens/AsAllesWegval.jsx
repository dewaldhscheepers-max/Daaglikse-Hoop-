...
export default function AsAllesWegval({ onClose }) {
  const [view, setView]             = useState('list')
  const [currentDay, setCurrentDay] = useState(null)
  const [dae, setDae]               = useState(cachedDae || [])
  const [loading, setLoading]       = useState(!cachedDae)
  const [completed, setCompleted]   = useState(() => {
    try { return JSON.parse(localStorage.getItem('aaw_completed') || '[]') } catch { return [] }
  })
  const [lastDay, setLastDay]       = useState(() => {
    const v = parseInt(localStorage.getItem('aaw_lastDay') || '0')
    return v || null
  })
  ...
  function openDay(dagNr) {
    ...
    if (!dag) return
    if (!lastDay && !localStorage.getItem('rp_counted_aaw')) {
      localStorage.setItem('rp_counted_aaw', '1')
      fetch('/api/kinder-boek-read', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ bookId: 'as-alles-wegval' }),
      }).catch(() => {})
    }
    setCurrentDay(dag)
    setLastDay(dagNr)
    localStorage.setItem('aaw_lastDay', String(dagNr))
    setView('day')
  }
        ...
        <div className="dvv-screen">
          ...
          <div className="dvv-list-body" ref={bodyRef}>
            <div className="bmg-intro">
              <div className="bmg-intro-sub">7 dae · Job, die Duiwel en die God van Herstel</div>
      ...
      <div className="dvv-overlay">
        <div className="dvv-screen" ref={bodyRef}>
          ...
          <div className="dvv-closing">
            ...
            <h1 className="dvv-closing-title">Jy het Job se pad saam met God gestap</h1>
            <p className="dvv-closing-body">
              Job het alles verloor — maar hy het God gevind op 'n dieper vlak as ooit tevore. Lyding is nie die einde van jou verhaal nie.
            </p>
            <p className="dvv-closing-body">
              Die God wat Job herstel het, is dieselfde God wat vir jou sorg. Hy sien jou pyn. Hy hoor jou roep. En Hy is besig om iets nuuts te bou — ook in jou.
            </p>
            <p className="dvv-closing-body">
              "Die Here het Job se lot verander toe hy vir sy vriende gebid het." — Job 42:10
            </p>
