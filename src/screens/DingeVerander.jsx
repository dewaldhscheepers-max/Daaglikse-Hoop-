...
export default function DingeVerander({ onClose }) {
  const [view, setView]           = useState('list')
  const [currentDay, setCurrentDay] = useState(null)
  const [dae, setDae]             = useState(cachedDae || [])
  const [completed, setCompleted] = useState(() => {
    try { return JSON.parse(localStorage.getItem('dvk_completed') || '[]') } catch { return [] }
  })
  const [lastDay, setLastDay]     = useState(() => {
    const v = parseInt(localStorage.getItem('dvk_lastDay') || '0')
    return v || null
  })
  ...
  useEffect(() => {
    if (view === 'day' && currentDay) {
      const saved = localStorage.getItem(`dvk_journal_${currentDay.dag}`) || ''
      setJournalText(saved)
    }
  }, [view, currentDay])

  function saveJournal(dagNr, text) {
    try { localStorage.setItem(`dvk_journal_${dagNr}`, text) } catch {}
  }
  ...
  function markCompleted(dagNr) {
    ...
    setCompleted(next)
    localStorage.setItem('dvk_completed', JSON.stringify(next))
  }

  function handleShare() {
    sharePlan('Dinge Wat Jou Lewe Kan Verander', `✨ Dinge Wat Jou Lewe Kan Verander — 24 dae\n\nLees dit gratis op Daaglikse Hoop:`)
  }
      ...
      <div className="dvv-overlay">
        <div className="dvv-screen" ref={bodyRef}>
          ...
          <div className="dvv-opening">
            ...
            <h1 className="dvv-opening-title">Dinge Wat Jou Lewe Kan Verander</h1>
            <p className="dvv-opening-sub">Dag-vir-dag waarhede wat jou lewe omvorm.</p>
            <p className="dvv-opening-desc">
              Stap deur dag-vir-dag waarhede wat jou help om anders te dink, bid en leef. Begin enige tyd. Geen slotjies nie.
            </p>
            {allDone ? (
              <>
      ...
      <div className="dvv-overlay">
        <div className="dvv-screen">
          <div className="dvv-header">
            ...
            <span className="dvv-header-title">Dinge Wat Jou Lewe Kan Verander</span>
